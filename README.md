<p align="center">
  <img src="./assets/railyard-appicon.png" alt="Railyard logo" width="180">
</p>
...
![Railyard showing a Workstream for reliable offline editing](./assets/railyard-product-screenshot.png)

Railyard is a local desktop app for working with [Pi](https://github.com/earendil-works/pi) across Git repositories and long-running goals. Plan, implement, and pick up where you left off.

> [!IMPORTANT]
> Railyard is currently in public beta. Back up important work, review Agent operations as carefully as shell commands, and expect occasional breaking changes before the stable release.
...
2. In PowerShell, verify the download:
   ...
   ```powershell
   $installer = Get-Item .\railyard-*-win-x64-setup.exe
...
2. Verify and install it:
   ...
   ```sh
   sha256sum --check railyard-*-amd64.sha256
   sudo apt install ./railyard-*-amd64.deb
   ```

3. Launch **Railyard** from your desktop application menu or run `railyard`.
...
Railyard uses Pi's existing model-provider configuration. It does not provide a separate account or provider login screen. Install [Pi CLI](https://pi.dev), then run `pi` and use `/login` to sign in to a provider and select a Model. Pi CLI stores the provider configuration that Railyard uses.

Railyard embeds Pi's runtime; Pi CLI is only required for its interactive provider setup. Do not copy credentials into Railyard or share `~/.pi/agent/auth.json`. See the [privacy and authority guide](./docs/privacy.md#pi-configuration-and-credentials) for details.
...
Railyard has no hosted account service or Railyard cloud data store. Application state and owned Session history are stored locally in your operating system's application-data directory.
...
Railyard does not add analytics, advertising identifiers, or a crash-reporting service. For storage locations, credentials, network transfer, retention, and the Agent's filesystem authority, read the full [privacy, data handling, and agent authority guide](./docs/privacy.md).
...
The browser demo uses fictional, in-memory data. It does not read or write Railyard data, call a model, make network requests, or open external links.
...
Except for the Catalyst-derived UI kit identified below, Railyard is licensed under the [Apache License, Version 2.0](./LICENSE).
