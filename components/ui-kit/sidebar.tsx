...
export function SidebarHeader({ className, ...props }: React.ComponentPropsWithoutRef<'div'>) {
  return (
    <div
      ...
      className={clsx(
        className,
        'flex flex-col border-b border-sidebar-border px-4 py-3 [&>[data-slot=section]+[data-slot=section]]:mt-2.5'
      )}
    />
  )
}
...
export function SidebarHeading({ className, ...props }: React.ComponentPropsWithoutRef<'h3'>) {
  return <h3 {...props} className={clsx(className, 'mb-1 px-2 text-xs/6 font-medium text-sidebar-muted-foreground')} />
}
...
export const SidebarItem = forwardRef(function SidebarItem(
  {
    ...
    children,
    ...props
  }: { current?: boolean; className?: string; currentIndicatorClassName?: string; children: React.ReactNode } & (
    ...
    <span className={clsx(className, 'relative')}>
      {current && (
        <span
          ...
          className={clsx(
            'absolute inset-y-2 -left-4 w-0.5 rounded-full',
            !currentIndicatorClassName && 'bg-sidebar-foreground',
            currentIndicatorClassName
          )}
        />
      )}
